  ...
  if(is.good())
    {
      ...
      while( is.getline(pName, 500, '\n') )
        {
          if (pName[0] == '#') continue;
	  if (pName[0] == ' ') continue; // simple protection against blank lines
	  if (pName[0] == '\n') continue;
...
int baseClass::getGlobalInfoNstart(char * pName)
{
  ...
  TFile f(pName);
  //f.cd();
  TTree * tree1; // shouldn't be necessary any more
  TTree * tree2;
  string s;
  s = *treeName_ + "_globalInfo";
  /*
  tree1 = (TTree*)gDirectory->Get("LJFilter");
  if (!tree1)
    {
      skimWasMade_ = false;
      return NBeforeSkim;
  ...
  */
  ...
  tree2= (TTree*)gDirectory->Get("LJFilter/EventCount");
  //tree2 = (TTree*)gDirectory->Get(s.c_str());
  if( !tree2 ) 
    {
      STDOUT("Directory 'LJFilter/EventCount' not found");
  ...
  // New way of getting the number of events prior to skimming
  TH1F* dummy = (TH1F*)gDirectory->Get("LJFilter/EventCount/EventCounter");
  if (dummy!=0)
    NBeforeSkim=(int)dummy->GetBinContent(1); // is there a more clever way to do this?

  //tree2->SetBranchAddress("EventCounter",&NBeforeSkim, &b_Nstart);
